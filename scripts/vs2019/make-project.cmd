...
"C:\Program Files (x86)\Microsoft Visual Studio\Installer\vswhere.exe" -version [16.0,17.0) -latest -property installationPath > %TEMP_FILE%
set /p INSTALLDIR= <%TEMP_FILE% > nul
del %TEMP_FILE%
set DEVENV=%INSTALLDIR%\Common7\IDE\devenv.exe
...
if %BUILD_WIN32_DEBUG%==Y (
   echo Making %SOLUTION_NAME%: %PROJECT_NAME% - "Debug|x86"
   "%DEVENV%" %SOLUTION_NAME% /Build "Debug|x86" /Project %PROJECT_NAME%
   if ERRORLEVEL 1 goto LABEL_ERROR
   )

if %BUILD_WIN32_RELEASE%==Y (
   echo Making %SOLUTION_NAME%: %PROJECT_NAME% - "Release|x86"
   "%DEVENV%" %SOLUTION_NAME% /Build "Release|x86" /Project %PROJECT_NAME%
   if ERRORLEVEL 1 goto LABEL_ERROR
   )
